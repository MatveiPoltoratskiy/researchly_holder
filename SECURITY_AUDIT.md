...
## Follow-up pass — 2026-09-06

Items #16 and #28 above (form input validation, missing rate limits on the write forms) were already superseded by later work not reflected in the sections above: `api/submit-waitlist.js` and `api/submit-contact.js` now front both tables with server-side validation, honeypot checks, and per-IP rate limiting, using a service-role key (`api/_supabaseAdmin.js`) that never ships to the client. `supabase/lockdown.sql` revokes the anon key's INSERT privilege on both tables, so the client-direct-to-Supabase write path described in the original #16/#28 findings no longer exists at all — confirmed no `@supabase/supabase-js` import or Supabase call remains anywhere under `src/`, and the anon key is no longer referenced client-side either.

Re-verified this session (all still hold): the lazy-loaded gate for `/interview`, `/opportunities`, `/my-opportunities` (finding #15) still keeps the dataset and interview code out of the main bundle and out of the network tab for an unauthenticated visitor — confirmed via a fresh production build + Puppeteer network trace. `npm audit`: 0 vulnerabilities.

New findings this pass:

1. **Rate limiter trusted a client-spoofable `X-Forwarded-For` position (FIXED).** `api/_rateLimit.js`'s `getClientIp()` read `x-forwarded-for`'s *first* entry, which is the value a client can set to anything before the request ever reaches Vercel's edge — every proxy in a forwarding chain is only supposed to *append* its own observed address, never rewrite what's already there. In practice this meant every rate limit in the app (`dev-unlock`'s passphrase brute-force protection included) could be bypassed by sending a different fake `X-Forwarded-For` value on each request. **Fixed:** now prefers `x-real-ip` (the single IP Vercel's own edge sets, not client-overridable), falling back to the *last* `x-forwarded-for` entry, then the raw socket address. No API contract or response shape changed — verified with unit-style calls covering all four header combinations, and a full `vite build` still succeeds.

2. **Waitlist email enumeration via `alreadyRegistered` (FOUND, not fixed — needs a product decision).** `api/submit-waitlist.js` returns `alreadyRegistered: true` when the submitted email hits the table's unique constraint, which `Footer.jsx` surfaces as "You're already subscribed." vs "You're subscribed." This lets anyone check whether a specific email address is on the waitlist by submitting it and reading the response — rate-limited (10/10min/IP) but not prevented, and trivially parallelized across IPs. Severity is low (no passwords or sensitive data behind this, just list membership of an email address), but it's a real, zero-effort information disclosure, not a theoretical one. Not fixed in this pass because the fix (always returning the same generic response/copy regardless of whether the row already existed) removes a piece of existing user-facing behavior — flagged for the site owner to decide rather than changed silently.
