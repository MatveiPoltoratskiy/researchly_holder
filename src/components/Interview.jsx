...
// a grid of the next 12 months (this month included) — picking one sets
// answers.applyStart/applyEnd to that month's first/last day, the same window shape
// scoreOpportunity already checks a listing's deadline against. Replaced the old
// drag-a-date-range calendar: a student planning when to apply thinks in months, not
// specific days, and picking one tile is faster than dragging across a day grid.
function MonthPicker({ selectedStartISO, onSelect }) {
  const start = startOfMonth(new Date())
  const months = Array.from({ length: 12 }, (_, i) => addMonths(start, i))
  return (
    <div className="interview-option-list interview-month-grid">
      {months.map((m) => {
        const iso = toISODate(m)
        const selected = iso === selectedStartISO
        return (
          <button
            type="button"
            key={iso}
            className={`interview-option-row interview-option-row--month ${selected ? 'is-selected' : ''}`}
            onClick={() => onSelect(m)}
          >
        ...
        <div className="interview-progress-row">
            ...
            <div className="interview-card" key={step}>
              <div className="interview-card-head">
                ...
                {step === 7 && (
                  <>
                    <h1 className="interview-question">Which month are you applying in?</h1>
                    <p className="interview-subtext">
                      Pick the month you'll realistically be working on applications. We'll prioritize matches whose
                      deadlines fall in that window.
                    </p>
                  </>
                )}
                {step === 8 && (
              ...
              <div className="interview-card-scroll">
                ...
                {step === 7 && (
                  <div className="interview-timeline">
                    <MonthPicker
                      selectedStartISO={answers.applyStart}
                      onSelect={(month) => {
                        const end = new Date(month.getFullYear(), month.getMonth(), daysInMonth(month))
                        setAnswers((a) => ({ ...a, applyStart: toISODate(month), applyEnd: toISODate(end) }))
                      }}
                    />
                    <div className="interview-timeline-footer">
                      {answers.applyStart ? (
                        <span className="interview-timeline-range">
                          ...
                          {MONTH_NAMES[fromISODate(answers.applyStart).getMonth()]} {fromISODate(answers.applyStart).getFullYear()}
                          <button
                            type="button"
